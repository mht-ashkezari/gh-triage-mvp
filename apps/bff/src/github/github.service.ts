...
export class GithubService {
    ...
    verifyHmac(sig256: string | undefined, rawBody: string | Buffer) {
        ...
        const expected = crypto.createHmac("sha256", this.webhookSecret).update(bodyBuf).digest("hex");
    ...
    async getInstallationToken(installationId: number): Promise<string> {
        // Lazily create the app auth only when needed
        if (!this.appId || !this.privateKey) {
            throw new Error("GitHub App credentials not configured");
        }
        const appAuth = createAppAuth({ appId: this.appId, privateKey: this.privateKey });
    ...
    async upsertRepos(installationId: number, repos: any[]) {
        ...
        try {
           ...
           values ($1,$2,$3,$4)
           on conflict (installation_id,owner,name) do update set private=excluded.private`,
    ...
    async listInstallations() {
        const { rows } = await pool.query(`select installation_id,account_login,account_type,suspended from installations order by installation_id asc`);
        return rows;
    }

    async listRepos(installationId: number) {
        const { rows } = await pool.query(
            `select installation_id, owner, name, private from repos where installation_id=$1 order by owner,name`,
            [installationId]
        );
        return rows;
    }
